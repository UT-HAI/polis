...
function mixedForce(arrays, strength) {
  ...
  const force = forceSimulation(allPoints).stop()
      ...
      .force("charge", d3Force.forceManyBody().strength(strength));
...
const graphUtil = (comments, math, badTids, ptptois) => {
    ...
    const xx = d3Scale.scaleLinear().domain([minClusterX, maxClusterX]).range([border, globals.side - border]);
    const yy = d3Scale.scaleLinear().domain([minClusterY, maxClusterY]).range([border, globals.svgHeightWithoutPadding - border]);
